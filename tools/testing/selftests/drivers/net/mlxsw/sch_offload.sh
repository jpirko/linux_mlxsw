...
with_ets()
{
	...
	tc qdisc add dev $h1 $locus handle $handle ets bands 8 priomap 7 6 5 4 3 2 1 0
	"$@"
...
with_prio()
{
	...
	tc qdisc add dev $h1 $locus handle $handle prio bands 8 priomap 7 6 5 4 3 2 1 0
	"$@"
	tc qdisc del dev $h1 $locus
}
...
with_red()
{
	...
	tc qdisc add dev $h1 $locus handle $handle red \
	   limit 1000000 min 200000 max 300000 probability 0.5 avpkt 1500
	"$@"
	tc qdisc del dev $h1 $locus
}
...
with_tbf()
{
	...
	tc qdisc add dev $h1 $locus handle $handle tbf rate 400Mbit burst 128K limit 1M
	"$@"
	tc qdisc del dev $h1 $locus
}
...
do_test_combinations()
{
	...
	do_test_offloaded $handle $parent red
	do_test_offloaded $handle $parent tbf
	do_test_offloaded $handle $parent pfifo
	do_test_offloaded $handle $parent bfifo
	...
	for cont in ets prio; do
		for leaf in "" red tbf "red tbf" "tbf red"; do
			for fifo in "" pfifo bfifo; do
				do_test_offloaded $handle $parent $cont $leaf $fifo
			done
		done
