...
# and relies on the implicit mlxsw configuration, where packet priority is taken
# 1:1 from the 802.1p marking.
#
# +-------------------------------------------+
# | H1                                        |
# |     + $h1.10                  $h1.11 +    |
...
{
	host_create $h2 2
	ip link set dev $h2.10 type vlan ingress 0:0 1:1
	ip link set dev $h2.11 type vlan ingress 0:0 1:1
}
...
tbf_get_counter()
{
	...
	ethtool_stats_get $h2 rx_octets_prio_$((vlan - 10))
}
