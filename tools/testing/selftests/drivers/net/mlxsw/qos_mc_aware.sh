...
# Thus we set MTU to 5K on all involved interfaces. Then both unicast and
# multicast traffic uses 4K frames.
#
# +-----------------------+                +----------------------------------+
# | H1                    |                |                               H2 |
# |                       |                |  unicast --> + $h2.111           |
...
measure_uc_rate()
{
	...
	mausezahn $h2.111 -p 4000 -A 192.0.2.129 -B 192.0.2.130 -c 0 \
...
test_mc_aware()
{
	...
	mausezahn $h1 -p 4000 -c 0 -a own -b bc -t udp -q &
