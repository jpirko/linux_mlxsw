...
get_qdisc_backlog()
{
	...
	qdisc_stats_get $(get_qdisc_handle $vlan) .backlog
}
...
get_qdisc_nmarked()
{
	...
	qdisc_stats_get $(get_qdisc_handle $vlan) .marked
}
...
get_qdisc_npackets()
{
	...
	qdisc_stats_get $(get_qdisc_handle $vlan) .packets
}
...
build_backlog()
{
	...
	while :; do
		local cur=$(get_qdisc_backlog $vlan)
		local diff=$((size - cur))
		...
		$MZ $h2.$vlan -p 8000 -a own -b $h3_mac \
		    -A $(ipaddr 1 $vlan) -B $(ipaddr 3 $vlan) \
		    -t $proto -q -c $pkts "$@"
		sleep 1
	done
}
...
check_marking()
{
	...
	local nmarked_0=$(get_qdisc_nmarked $vlan)
	local npackets_0=$(get_qdisc_npackets $vlan)
	sleep 5
	local nmarked_1=$(get_qdisc_nmarked $vlan)
