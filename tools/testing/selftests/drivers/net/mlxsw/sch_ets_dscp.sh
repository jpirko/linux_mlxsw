...
ets_change_qdisc()
{
	...
	tc qdisc $op dev $dev root handle 10: ets			       \
...
collect_stats()
{
	...
	for stream in ${streams[@]}; do
		tc_rule_stats_get $h4 100$stream
	done
}
...
switch_create()
{
	...
	dcb app add dev $swp1 dscp-prio $TOS:0
	dcb app add dev $swp2 dscp-prio $TOS:1
	dcb app add dev $swp3 dscp-prio $TOS:2
...
switch_destroy()
{
	...
	dcb app del dev $swp3 dscp-prio $TOS:2
	dcb app del dev $swp2 dscp-prio $TOS:1
	dcb app del dev $swp1 dscp-prio $TOS:0
