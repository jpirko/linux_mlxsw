...
# This is a very fickle test for SW-datapath behavior of RED. It sends one
# stream of traffic from H1 through a TBF shaper, to a RED within TBF shaper on
# $swp3. The two shapers have the same configuration, and thus the resulting
# stream should fill all available bandwidth on the latter shaper. A second
# stream is sent from H2 also via $swp3, and used to inject additional traffic.
# Since all available bandwidth is taken, this traffic has to go to backlog.
#
# +--------------------------+                     +--------------------------+
# | H1                       |                     | H2                       |
# |     + $h1                |                     |     + $h2                |
...
# It would seem that these values can be freely chosen, but unfortunately that
# is not so. This is one of the few configuration space points where the test
# passes.
BACKLOG=30000
PKTSZ=1400
...
build_backlog()
{
	...
	while :; do
		...
		$MZ $h2 -p $PKTSZ -a own -b $h3_mac \
		    -A 192.0.2.2 -B 192.0.2.3 \
		    -t $proto -q -c $pkts "$@"
		sleep 1
	done
}
