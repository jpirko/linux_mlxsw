...
size_t br_vlan_opts_nl_size(void)
{
	return nla_total_size(sizeof(u8)) /* BRIDGE_VLANDB_ENTRY_STATE */
	       ...
	       + nla_total_size(sizeof(u32)) /* BRIDGE_VLANDB_ENTRY_MAX_GROUPS */
#endif
	       + 0;
}
				    ...
				    struct netlink_ext_ack *extack)
{
	...
	*changed = false;
	...
	if (tb[BRIDGE_VLANDB_ENTRY_MAX_GROUPS]) {
		u32 val;

		val = nla_get_u32(tb[BRIDGE_VLANDB_ENTRY_MAX_GROUPS]);
		// xxx const cast :-(
		err = br_multicast_port_ngroups_set_max((struct net_bridge_port *) p,
							v, val, extack);
		if (err)
			return err;
		*changed = true;
