					...
					struct dcb_ieee_app_prio_map *p_map)
{
	...
	spin_lock_bh(&dcb_lock);
	list_for_each_entry(itr, &dcb_app_list, list) {
		if (itr->app.selector == IEEE_8021QAZ_APP_SEL_DSCP &&
		    itr->app.protocol < 64 &&
		    itr->app.priority < ARRAY_SIZE(p_map->map) &&
		    itr->ifindex == ifindex) {
			prio = itr->app.priority;
				   ...
				   struct dcb_ieee_app_dscp_map *p_map)
{
	...
	list_for_each_entry(itr, &dcb_app_list, list) {
		if (itr->app.selector == IEEE_8021QAZ_APP_SEL_DSCP &&
		    itr->app.priority < 8 &&
		    itr->app.protocol < ARRAY_SIZE(p_map->map) &&
		    itr->ifindex == ifindex)
...
u8 dcb_ieee_getapp_default_prio_mask(const struct net_device *dev)
{
	...
	spin_lock_bh(&dcb_lock);
	list_for_each_entry(itr, &dcb_app_list, list) {
		if (itr->app.selector == IEEE_8021QAZ_APP_SEL_ETHERTYPE &&
		    itr->app.protocol == 0 &&
		    itr->ifindex == ifindex)
