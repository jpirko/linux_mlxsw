...
static int devlink_verify_generic_param(const struct devlink_param *param)
{
	int i;

	for (i = 0; i < DEVLINK_PARAM_GENERIC_MAX; i++)
		if (!strcmp(param->name, devlink_param_generic[i].name))
			break;
	/* not found - not a generic parameter */
	if (i == DEVLINK_PARAM_GENERIC_MAX)
		return -ENOENT;

	WARN_ON(param->type != devlink_param_generic[i].type);

	return 0;
}

static int devlink_verify_driver_param(const char *param_name)
{
	int i;

	/* verify no such name in generic params */
	for (i = 0; i < DEVLINK_PARAM_GENERIC_MAX; i++)
		if (!strcmp(param_name, devlink_param_generic[i].name))
			return -EEXIST;
...
static bool devlink_config_mode_is_supported(struct devlink_param *param,
					     enum devlink_config_mode mode)
{
	return test_bit(mode, &param->config_modes);
}
...
static int
devlink_nl_param_fill_one_value(struct sk_buff *msg,
				...
				union devlink_param_value val)
{
	...
	if (nla_put_u8(msg, DEVLINK_ATTR_PARAM_VALUE_MODE, mode))
	...
	case DEVLINK_PARAM_TYPE_U32:
		...
		break;
	case DEVLINK_PARAM_TYPE_S8:
		if (nla_put_s8(msg, DEVLINK_ATTR_PARAM_VALUE_DATA, val.vs8))
			goto value_nest_cancel;
		break;
	case DEVLINK_PARAM_TYPE_S32:
		if (nla_put_s32(msg, DEVLINK_ATTR_PARAM_VALUE_DATA, val.vs32))
			goto value_nest_cancel;
		break;
	case DEVLINK_PARAM_TYPE_STRING:
...
static int devlink_nl_param_fill(struct sk_buff *msg, struct devlink *devlink,
				 ...
				 u32 portid, u32 seq, int flags)
{
	union devlink_param_value param_value[DEVLINK_CONFIG_MODE_MAX + 1];
	struct devlink_param *param = param_item->param;
	struct devlink_param_gsetter_ctx ctx;
	...
	/* Get value from driver part to driver init mode */
	for (i = 0; i <= DEVLINK_CONFIG_MODE_MAX; i++) {
		if (!devlink_config_mode_is_supported(param, i))
			continue;
		if (i == DEVLINK_CONFIG_MODE_DRIVER_INIT) {
			if (!param_item->driver_init_value_valid)
				return -EOPNOTSUPP;
			param_value[i] = param_item->driver_init_value;
		} else {
			ctx.config_mode = i;
	...
	for (i = 0; i <= DEVLINK_CONFIG_MODE_MAX; i++) {
		if (!devlink_config_mode_is_supported(param, i))
			continue;
		err = devlink_nl_param_fill_one_value(msg, param->type,
						      i, param_value[i]);
		if (err)
				      ...
				      const struct devlink_param *param)
{
	...
	if (param->config_modes == 1UL << DEVLINK_CONFIG_MODE_DRIVER_INIT)
		WARN_ON(param->getter || param->setter);
	else
		WARN_ON(!param->getter || !param->setter);
	...
	if (!param_item)
		return -ENOMEM;
	param_item->param = kmemdup(param, sizeof(*param), GFP_KERNEL);
	if (!param_item->param) {
		err = -ENOMEM;
		goto err_param_kmemdup;
	}
			    ...
			    size_t params_count)
{
	...
	for (i = 0; i < params_count; i++, param++) {
		if (!param || !param->name || !param->config_modes) {
			err = -EINVAL;
			goto unregister;
		}
		if (param->generic) {
			err = devlink_verify_generic_param(param);
			if (err)
				goto unregister;
		} else {
			err = devlink_verify_driver_param(param->name);
			if (err)
				goto unregister;
		}
...
unregister:
	for (j = 0, param = params; j < i; j++, param++)
