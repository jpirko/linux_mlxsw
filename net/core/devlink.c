...
{
	if (nla_put_u16(msg, DEVLINK_ATTR_PORT_TYPE, port_type))
		return -EMSGSIZE;

	if (port_type == DEVLINK_PORT_TYPE_ETH) {
		struct net_device *netdev;

		netdev = rcu_dereference_check(devlink_port->type_dev,
				lockdep_is_held(&devlink_port->type_lock));
		if (netdev &&
	...
	if (port_type == DEVLINK_PORT_TYPE_IB) {
		struct ib_device *ibdev;

		ibdev = rcu_dereference_check(devlink_port->type_dev,
				lockdep_is_held(&devlink_port->type_lock));
		if (ibdev &&
				...
				u32 seq, int flags)
{
	...
	if (devlink_nl_port_type_fill(msg, devlink_port, devlink_port->type))
		goto nla_put_failure_type_locked;
...
static u32 devlink_trap_obj_hashfn(const void *data, u32 len, u32 seed)
{
	const struct devlink_trap_item *trap_item = data;

	return jhash(&trap_item->trap->id, len, seed);
}

static int devlink_trap_obj_cmpfn(struct rhashtable_compare_arg *arg,
				  const void *obj)
{
	const struct devlink_trap_item *trap_item = obj;
	u16 trap_id = *(u16 *) arg->key;

	return trap_item->trap->id != trap_id;
}

static const struct rhashtable_params devlink_trap_ht_params = {
	.head_offset = offsetof(struct devlink_trap_item, ht_node),
	.key_len = sizeof(u16),
	.obj_hashfn = devlink_trap_obj_hashfn,
	.obj_cmpfn = devlink_trap_obj_cmpfn,
};
				...
				struct genl_info *info)
{
	...
	list_for_each_entry(trap_item, &devlink->trap_list, list) {
		if (nla_strcmp(attr, trap_item->trap->name) == 0)
			return trap_item;
	}
...
static int
devlink_trap_report_get_from_info(struct genl_info *info,
				  enum devlink_trap_report *p_trap_report)
{
	u8 val;

	val = nla_get_u8(info->attrs[DEVLINK_ATTR_TRAP_REPORT]);
	switch (val) {
	case DEVLINK_TRAP_REPORT_OFF: /* fall-through */
				...
				int flags)
{
	...
	if (nla_put_u8(msg, DEVLINK_ATTR_TRAP_REPORT, trap_item->report))
		goto nla_put_failure;
				     ...
				     struct netlink_ext_ack *extack)
{
	...
	err = trap_item->trap->action_set(devlink, trap_id, trap_action,
					  extack);
	if (err)
		return err;
...
static int devlink_trap_report_set(struct devlink *devlink,
				   struct devlink_trap_item *trap_item,
				   struct genl_info *info)
{
					...
					struct genl_info *info)
{
	...
	err = devlink_trap_report_set(devlink, trap_item, info);
	if (err)
		return err;
...
devlink_trap_group_item_lookup(struct devlink *devlink, const char *name)
{
	...
	list_for_each_entry(group_item, &devlink->trap_group_list, list) {
		if (strcmp(group_item->group->name, name) == 0)
			return group_item;
	}
				...
				struct netlink_ext_ack *extack)
{
	...
	list_for_each_entry(trap_item, &devlink->trap_list, list) {
		if (strcmp(trap_item->trap->group_name, group_name))
			continue;
...
#define DEVLINK_TRAP_GENERIC(_id, _type)			\
{								\
...
static const struct devlink_trap devlink_trap_generic[] = {
	DEVLINK_TRAP_GENERIC(INGRESS_SMAC_MC_DROP, DROP),
	DEVLINK_TRAP_GENERIC(INGRESS_VLAN_TAG_ALLOW_DROP, DROP),
	DEVLINK_TRAP_GENERIC(INGRESS_VLAN_FILTER_DROP, DROP),
	DEVLINK_TRAP_GENERIC(INGRESS_STP_FILTER_DROP, DROP),
	DEVLINK_TRAP_GENERIC(EMPTY_TX_LIST_DROP, DROP),
	DEVLINK_TRAP_GENERIC(LOOPBACK_FILTER_DROP, DROP),
	DEVLINK_TRAP_GENERIC(BLACKHOLE_ROUTE_DROP, DROP),
	DEVLINK_TRAP_GENERIC(TTL_ERROR_EXCEPTION, EXCEPTION),
	DEVLINK_TRAP_GENERIC(TAIL_DROP, DROP),
	DEVLINK_TRAP_GENERIC(EARLY_DROP, DROP),
};
...
static int devlink_trap_driver_verify(const struct devlink_trap *trap)
{
	...
	for (i = 0; i < ARRAY_SIZE(devlink_trap_generic); i++) {
		if (strcmp(trap->name, devlink_trap_generic[i].name) == 0)
			return -EEXIST;
	}
...
{
	if (!trap || !trap->action_set || !trap->name)
		return -EINVAL;

	if (trap->metadata_cap.in_port &&
	    trap->metadata_cap.port_type != DEVLINK_PORT_TYPE_ETH &&
	    trap->metadata_cap.port_type != DEVLINK_PORT_TYPE_IB)
		return -EINVAL;
...
devlink_trap_item_group_assoc(struct devlink *devlink,
			      struct devlink_trap_item *trap_item)
{
	const char *group_name = trap_item->trap->group_name;
	struct devlink_trap_group_item *group_item;

	/* Groups are optional */
	if (list_empty(&devlink->trap_group_list) || !group_name)
		return 0;

	group_item = devlink_trap_group_item_lookup(devlink, group_name);
...
static int devlink_trap_register(struct devlink *devlink,
				 const struct devlink_trap *trap)
{
	struct devlink_trap_item *trap_item;
	int err;

	if (rhashtable_lookup_fast(&devlink->trap_ht, &trap->id,
	...
	trap_item->report = DEVLINK_TRAP_REPORT_OFF;
				    ...
				    const struct devlink_trap *trap)
{
	...
	trap_item = rhashtable_lookup_fast(&devlink->trap_ht, &trap->id,
					   devlink_trap_ht_params);
				 ...
				 const struct devlink_trap *trap)
{
	...
	trap_item->trap->action_set(devlink, trap_item->trap->id,
				    DEVLINK_TRAP_ACTION_DROP, NULL);
}
			   ...
			   size_t traps_count)
{
	...
	for (i = 0; i < traps_count; i++) {
		...
		err = devlink_trap_register(devlink, trap);
		if (err)
...
devlink_nl_trap_group_size(const struct devlink_trap_group_item *group_item)
{
	...
	return nla_total_size(strlen(group_item->group->name) + 1);
}
				   ...
				   const struct devlink_port *in_devlink_port)
{
	...
	if (trap->metadata_cap.in_port == 0)
		return 0;
			 ...
			 const struct devlink_port *in_devlink_port)
{
	...
	trap_item = rhashtable_lookup_fast(&devlink->trap_ht, &trap_id,
					   devlink_trap_ht_params);
	if (!trap_item)
		goto unlock;
