...
static int net_dm_monitor_start(bool change_sw, bool change_hw,
...
static void net_dm_monitor_stop(bool change_sw, bool change_hw,
				struct netlink_ext_ack *extack)
{
	if (change_hw)
...
static int net_dm_cmd_trace(struct sk_buff *skb,
			struct genl_info *info)
{
	bool change_sw = !!info->attrs[NET_DM_ATTR_SW];
	bool change_hw = !!info->attrs[NET_DM_ATTR_HW];
	struct netlink_ext_ack *extack = info->extack;

	/* To maintain backward compatibility, we start / stop monitoring of
	 * software drops if no flag is specified.
	 */
	if (!change_sw && !change_hw)
		change_sw = true;

	switch (info->genlhdr->cmd) {
	case NET_DM_CMD_START:
		return net_dm_monitor_start(change_sw, change_hw, extack);
	case NET_DM_CMD_STOP:
		net_dm_monitor_stop(change_sw, change_hw, extack);
		return 0;
	}
...
static const struct nla_policy net_dm_nl_policy[NET_DM_ATTR_MAX + 1] = {
	...
	[NET_DM_ATTR_SW]	= {. type = NLA_FLAG },
	[NET_DM_ATTR_HW]	= {. type = NLA_FLAG },
};
