			      ...
			      struct mlxsw_afk_element_usage *elusage)
{
	...
	mlxsw_sp_acl_tcam_group_use_patterns(group, elusage, &vregion_elusage);

	return mlxsw_sp_acl_tcam_vregion_create(mlxsw_sp, group, priority,
						&vregion_elusage);
}
				...
				struct mlxsw_afk_element_usage *elusage)
{
	...
	vregion = mlxsw_sp_acl_tcam_vregion_get(mlxsw_sp, group,
						priority, elusage);
	if (IS_ERR(vregion)) {
		err = PTR_ERR(vregion);
