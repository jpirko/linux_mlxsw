...
/* L4 Port Action
 * --------------
 * The L4_PORT_ACTION is used for modifying the sport and dport fields of the
 * packet, e.g. for NAT. If (the L4 is TCP) or if (the L4 is UDP and checksum
 * field!=0) then the L4 checksum is updated.
 */
...
static void mlxsw_afa_l4port_pack(char *payload, enum mlxsw_afa_l4port_s_d s_d,
				  u16 l4_port)
{
...
int mlxsw_afa_block_append_l4port(struct mlxsw_afa_block *block,
				  bool is_dport, u16 l4_port,
				  struct netlink_ext_ack *extack)
{
