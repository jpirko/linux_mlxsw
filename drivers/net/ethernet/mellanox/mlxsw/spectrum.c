...
{
	struct mlxsw_sp *mlxsw_sp = mlxsw_core_driver_priv(mlxsw_core);
	int split_width = mlxsw_sp->port_module_max_width / count;
			       ...
			       struct netlink_ext_ack *extack)
{
	...
	if (count == 1 || !is_power_of_2(count) ||
	    count > mlxsw_sp->port_module_max_width) {
		netdev_err(mlxsw_sp_port->dev, "Invalid split count\n");
		NL_SET_ERR_MSG_MOD(extack, "Invalid split count");
		return -EINVAL;
	}

	offset = mlxsw_sp_local_ports_offset(mlxsw_core, count);
	if (offset < 0) {
	 ...
	 */
	base_port = count == mlxsw_sp->port_module_max_width ?
		    mlxsw_sp_cluster_base_port_get(mlxsw_sp, local_port) :
		    local_port;
