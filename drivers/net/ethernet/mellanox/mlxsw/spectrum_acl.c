...
#define MLXSW_SP_ACL_MANGLE_ACTION_TCP(_offset, _mask, _shift, _field) \
	MLXSW_SP_ACL_MANGLE_ACTION(FLOW_ACT_MANGLE_HDR_TYPE_TCP,       \
				   _offset, _mask, _shift, _field)

#define MLXSW_SP_ACL_MANGLE_ACTION_UDP(_offset, _mask, _shift, _field) \
	MLXSW_SP_ACL_MANGLE_ACTION(FLOW_ACT_MANGLE_HDR_TYPE_UDP,       \
				   _offset, _mask, _shift, _field)
...
static int
mlxsw_sp1_acl_rulei_act_mangle_field(struct mlxsw_sp *mlxsw_sp,
				     struct mlxsw_sp_acl_rule_info *rulei,
				     struct mlxsw_sp_acl_mangle_action *mact,
				     u32 val, struct netlink_ext_ack *extack)
{
	...
	err = mlxsw_sp_acl_rulei_act_mangle_field(mlxsw_sp, rulei, mact,
						  val, extack);
	if (err != -EOPNOTSUPP)
		return err;
...
static int
mlxsw_sp2_acl_rulei_act_mangle_field(struct mlxsw_sp *mlxsw_sp,
				     struct mlxsw_sp_acl_rule_info *rulei,
				     struct mlxsw_sp_acl_mangle_action *mact,
				     u32 val, struct netlink_ext_ack *extack)
{
	...
	case MLXSW_SP_ACL_MANGLE_FIELD_IP_SPORT:
		return mlxsw_afa_block_append_l4port(rulei->act_block, false,
						     val, extack);
	case MLXSW_SP_ACL_MANGLE_FIELD_IP_DPORT:
		return mlxsw_afa_block_append_l4port(rulei->act_block, true,
						     val, extack);
	default:
		break;
	}
...
int mlxsw_sp_acl_rulei_act_mangle(struct mlxsw_sp *mlxsw_sp,
				  ...
				  struct netlink_ext_ack *extack)
{
	const struct mlxsw_sp_rulei_ops *rulei_ops = mlxsw_sp->rulei_ops;
	struct mlxsw_sp_acl_mangle_action *mact;
	size_t i;
	...
	for (i = 0; i < ARRAY_SIZE(mlxsw_sp_acl_mangle_actions); ++i) {
		...
		if (mact->htype == htype &&
		    ...
		    mact->mask == mask) {
			val >>= mact->shift;
			return rulei_ops->act_mangle_field(mlxsw_sp, rulei,
							   mact, val, extack);
		}
	}
...
struct mlxsw_sp_rulei_ops mlxsw_sp1_rulei_ops = {
	.act_mangle_field = mlxsw_sp1_acl_rulei_act_mangle_field,
};

struct mlxsw_sp_rulei_ops mlxsw_sp2_rulei_ops = {
