					       ...
					       bool force)
{
	...
	/* Clear the activity on both the old and new payload as it is
...
mlxsw_sp_nexthop_obj_bucket_adj_update(struct mlxsw_sp *mlxsw_sp,
				       ...
				       struct nh_notifier_info *info)
{
	u32 bucket_index = info->nh_res_bucket->bucket_index;
	struct netlink_ext_ack *extack = info->extack;
	...
	err = __mlxsw_sp_nexthop_update(mlxsw_sp, adj_index, nh, force,
					ratr_pl);
	if (err) {
