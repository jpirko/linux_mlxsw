...
static struct mlxsw_sp_fib_node_priv *
mlxsw_sp_fib_node_priv_create(const struct mlxsw_sp_router_ll_ops *ll_ops)
{
	struct mlxsw_sp_fib_node_priv *priv;

	if (!ll_ops->fib_node_priv_size)
		/* No need to have priv */
		return NULL;

	priv = kzalloc(sizeof(*priv) + ll_ops->fib_node_priv_size, GFP_KERNEL);
	if (!priv)
...
static void
mlxsw_sp_fib_node_priv_destroy(struct mlxsw_sp_fib_node_priv *priv)
{
	kfree(priv);
}

static void mlxsw_sp_fib_node_priv_hold(struct mlxsw_sp_fib_node_priv *priv)
{
	refcount_inc(&priv->refcnt);
}

static void mlxsw_sp_fib_node_priv_put(struct mlxsw_sp_fib_node_priv *priv)
{
	if (!priv || !refcount_dec_and_test(&priv->refcnt))
		return;
	mlxsw_sp_fib_node_priv_destroy(priv);
}
					       ...
					       struct mlxsw_sp_fib_node_priv *priv)
{
	if (!priv)
		return;
	mlxsw_sp_fib_node_priv_hold(priv);
	list_add(&priv->list, &op_ctx->fib_node_priv_list);
}

static void mlxsw_sp_fib_node_op_ctx_priv_put_all(struct mlxsw_sp_fib_node_op_ctx *op_ctx)
{
	struct mlxsw_sp_fib_node_priv *priv, *tmp;

	list_for_each_entry_safe(priv, tmp, &op_ctx->fib_node_priv_list, list)
...
struct mlxsw_sp_fib_node_op_ctx_basic {
...
mlxsw_sp_router_ll_basic_fib_node_pack(struct mlxsw_sp_fib_node_op_ctx *op_ctx,
				       ...
				       struct mlxsw_sp_fib_node_priv *priv)
{
	struct mlxsw_sp_fib_node_op_ctx_basic *op_ctx_basic = (void *) op_ctx->ll_priv;
	...
	case MLXSW_SP_L3_PROTO_IPV6:
		mlxsw_reg_ralue_pack6(ralue_pl, ralue_op, virtual_router, type,
				      bmp_len, prefix_len, &addr->addr6);
		break;
	}
}

static void
mlxsw_sp_router_ll_basic_fib_entry_act_remote_pack(struct mlxsw_sp_fib_node_op_ctx *op_ctx,
...
static void
mlxsw_sp_router_ll_basic_fib_entry_act_local_pack(struct mlxsw_sp_fib_node_op_ctx *op_ctx,
						  enum mlxsw_reg_ralue_trap_action trap_action,
...
static void
mlxsw_sp_router_ll_basic_fib_entry_act_ip2me_pack(struct mlxsw_sp_fib_node_op_ctx *op_ctx)
{
	struct mlxsw_sp_fib_node_op_ctx_basic *op_ctx_basic = (void *) op_ctx->ll_priv;
...
static void
mlxsw_sp_router_ll_basic_fib_entry_act_ip2me_tun_pack(struct mlxsw_sp_fib_node_op_ctx *op_ctx,
						      u32 tunnel_ptr)
{
	struct mlxsw_sp_fib_node_op_ctx_basic *op_ctx_basic = (void *) op_ctx->ll_priv;
...
static int
mlxsw_sp_router_ll_basic_fib_node_commit(struct mlxsw_sp *mlxsw_sp,
...
static bool
mlxsw_sp_router_ll_basic_fib_node_is_committed(struct mlxsw_sp_fib_node_priv *priv)
{
	return true;
}
				    ...
				    const struct mlxsw_sp_router_ll_ops *ll_ops)
{
	...
	err = ll_ops->fib_node_commit(mlxsw_sp, op_ctx, &postponed_for_bulk);
	if (!postponed_for_bulk)
		mlxsw_sp_fib_node_op_ctx_priv_put_all(op_ctx);
	return err;
}
...
static void mlxsw_sp_fib_entry_op_local(struct mlxsw_sp *mlxsw_sp,
					struct mlxsw_sp_fib_node_op_ctx *op_ctx,
					struct mlxsw_sp_fib_entry *fib_entry)
{
	const struct mlxsw_sp_router_ll_ops *ll_ops = fib_entry->fib_node->fib->ll_ops;
...
static void mlxsw_sp_fib_entry_op_trap(struct mlxsw_sp *mlxsw_sp,
				       struct mlxsw_sp_fib_node_op_ctx *op_ctx,
				       struct mlxsw_sp_fib_entry *fib_entry)
{
	const struct mlxsw_sp_router_ll_ops *ll_ops = fib_entry->fib_node->fib->ll_ops;
...
static void mlxsw_sp_fib_entry_op_blackhole(struct mlxsw_sp *mlxsw_sp,
					    struct mlxsw_sp_fib_node_op_ctx *op_ctx,
					    struct mlxsw_sp_fib_entry *fib_entry)
{
	const struct mlxsw_sp_router_ll_ops *ll_ops = fib_entry->fib_node->fib->ll_ops;
...
static void mlxsw_sp_fib_entry_op_nve_decap(struct mlxsw_sp *mlxsw_sp,
					    struct mlxsw_sp_fib_node_op_ctx *op_ctx,
					    struct mlxsw_sp_fib_entry *fib_entry)
{
	const struct mlxsw_sp_router_ll_ops *ll_ops = fib_entry->fib_node->fib->ll_ops;
				   ...
				   struct mlxsw_sp_fib_entry *fib_entry)
{
	switch (fib_entry->type) {
	case MLXSW_SP_FIB_ENTRY_TYPE_REMOTE:
		return mlxsw_sp_fib_entry_op_remote(mlxsw_sp, op_ctx, fib_entry);
	case MLXSW_SP_FIB_ENTRY_TYPE_LOCAL:
		mlxsw_sp_fib_entry_op_local(mlxsw_sp, op_ctx, fib_entry);
		break;
	case MLXSW_SP_FIB_ENTRY_TYPE_TRAP:
		mlxsw_sp_fib_entry_op_trap(mlxsw_sp, op_ctx, fib_entry);
		break;
	case MLXSW_SP_FIB_ENTRY_TYPE_BLACKHOLE:
		mlxsw_sp_fib_entry_op_blackhole(mlxsw_sp, op_ctx, fib_entry);
		break;
	case MLXSW_SP_FIB_ENTRY_TYPE_UNREACHABLE:
		mlxsw_sp_fib_entry_op_unreachable(mlxsw_sp, op_ctx, fib_entry);
		break;
	case MLXSW_SP_FIB_ENTRY_TYPE_IPIP_DECAP:
		return mlxsw_sp_fib_entry_op_ipip_decap(mlxsw_sp, op_ctx, fib_entry);
	case MLXSW_SP_FIB_ENTRY_TYPE_NVE_DECAP:
		mlxsw_sp_fib_entry_op_nve_decap(mlxsw_sp, op_ctx, fib_entry);
		break;
	default:
		return -EINVAL;
	}
...
static int mlxsw_sp_fib_entry_op(struct mlxsw_sp *mlxsw_sp,
				 ...
				 enum mlxsw_sp_fib_node_op op)
{
	int err = __mlxsw_sp_fib_entry_op(mlxsw_sp, op_ctx, fib_entry);
...
static int mlxsw_sp_fib_node_op(struct mlxsw_sp *mlxsw_sp,
				struct mlxsw_sp_fib_node_op_ctx *op_ctx,
				struct mlxsw_sp_fib_node *fib_node,
				enum mlxsw_sp_fib_node_op op)
{
...
static int __mlxsw_sp_fib_node_update(struct mlxsw_sp *mlxsw_sp,
				      struct mlxsw_sp_fib_node_op_ctx *op_ctx,
				      struct mlxsw_sp_fib_node *fib_node,
				      bool is_new)
{
	return mlxsw_sp_fib_node_op(mlxsw_sp, op_ctx, fib_node,
				    is_new ? MLXSW_SP_FIB_NODE_OP_WRITE :
				    MLXSW_SP_FIB_NODE_OP_UPDATE);
}

static int mlxsw_sp_fib_node_update(struct mlxsw_sp *mlxsw_sp,
				    struct mlxsw_sp_fib_node *fib_node)
{
	struct mlxsw_sp_fib_node_op_ctx *op_ctx = mlxsw_sp->router->ll_op_ctx;

	mlxsw_sp_fib_node_op_ctx_clear(op_ctx);
	return __mlxsw_sp_fib_node_update(mlxsw_sp, op_ctx, fib_node, false);
}

static int mlxsw_sp_fib_node_del(struct mlxsw_sp *mlxsw_sp,
				 struct mlxsw_sp_fib_node_op_ctx *op_ctx,
				 struct mlxsw_sp_fib_node *fib_node)
{
	const struct mlxsw_sp_router_ll_ops *ll_ops = fib_node->fib->ll_ops;
...
static int mlxsw_sp_fib_entry_update(struct mlxsw_sp *mlxsw_sp,
				     struct mlxsw_sp_fib_entry *fib_entry)
{
	return mlxsw_sp_fib_node_update(mlxsw_sp, fib_entry->fib_node);
}
			 ...
			 unsigned char prefix_len)
{
	...
	fib_node->priv = mlxsw_sp_fib_node_priv_create(fib->ll_ops);
	if (IS_ERR(fib_node->priv)) {
		err = PTR_ERR(fib_node->priv);
		goto err_fib_node_priv_create;
	}
...
static void mlxsw_sp_fib_node_destroy(struct mlxsw_sp_fib_node *fib_node)
{
	...
	mlxsw_sp_fib_node_priv_put(fib_node->priv);
	kfree(fib_node);
}
					  ...
					  struct mlxsw_sp_fib6_entry *fib6_entry)
{
	 ...
	 */
	err = __mlxsw_sp_fib_node_update(mlxsw_sp, op_ctx,
					 fib6_entry->common.fib_node, false);
	if (err)
		goto err_fib_entry_update;
					    ...
					    u8 tree_id)
{
	...
	for (i = 0; i < MLXSW_CORE_RES_GET(mlxsw_sp->core, MAX_VRS); i++) {
		...
		priv = mlxsw_sp_fib_node_priv_create(ll_ops);
		if (IS_ERR(priv))
			return PTR_ERR(priv);
...
static void mlxsw_sp_router_fib_event_work(struct work_struct *work)
{
	...
	struct mlxsw_sp_fib_node_op_ctx *op_ctx = router->ll_op_ctx;
	...
	WARN_ON_ONCE(!list_empty(&router->ll_op_ctx->fib_node_priv_list));
	mutex_unlock(&router->lock);
}
...
static const struct mlxsw_sp_router_ll_ops mlxsw_sp_router_ll_basic_ops = {
	...
	.fib_node_op_ctx_size = sizeof(struct mlxsw_sp_fib_node_op_ctx_basic),
	.fib_node_pack = mlxsw_sp_router_ll_basic_fib_node_pack,
...
static int mlxsw_sp_router_ll_op_ctx_init(struct mlxsw_sp_router *router)
{
	...
	for (i = 0; i < MLXSW_SP_L3_PROTO_MAX; i++) {
		size_t size = router->proto_ll_ops[i]->fib_node_op_ctx_size;
	...
	if (!router->ll_op_ctx)
		return -ENOMEM;
	INIT_LIST_HEAD(&router->ll_op_ctx->fib_node_priv_list);
	return 0;
}

static void mlxsw_sp_router_ll_op_ctx_fini(struct mlxsw_sp_router *router)
{
	WARN_ON(!list_empty(&router->ll_op_ctx->fib_node_priv_list));
