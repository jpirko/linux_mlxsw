...
int mlxsw_sp_pbs_autoresize(struct mlxsw_sp_port *mlxsw_sp_port, struct mlxsw_sp_pb *pb)
{
	...
	for (i = 0; i < DCBX_MAX_BUFFERS; i++) {
		if (!mlxsw_sp_pb_is_used(pb, i)) {
			...
			pb->buffer[i].thres_cells = 2 * mlxsw_sp_bytes_cells(mlxsw_sp, pb->mtu);
			pb->buffer[i].size_cells = pb->buffer[i].thres_cells;
...
static int mlxsw_sp_port_pb_init(struct mlxsw_sp_port *mlxsw_sp_port)
{
	...
	mlxsw_sp_port->pb = kzalloc(sizeof(*mlxsw_sp_port->pb), GFP_KERNEL);
	if (!mlxsw_sp_port->pb)
		return -ENOMEM;
	...
	err = __mlxsw_sp_pbs_configure(mlxsw_sp_port, &pb, true);
	if (err)
...
int mlxsw_sp_port_buffers_init(struct mlxsw_sp_port *mlxsw_sp_port)
{
	...
	if (err)
		return err;
	return mlxsw_sp_port_pb_init(mlxsw_sp_port);
}
