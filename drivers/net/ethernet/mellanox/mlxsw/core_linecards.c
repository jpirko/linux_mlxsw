					  ...
					  struct netlink_ext_ack *extack)
{
	struct mlxsw_linecard_device_fw_info info = {
		.mlxfw_dev = {
			...
			.psid = device->info.psid,
			.psid_size = strlen(device->info.psid),
			.devlink = priv_to_devlink(mlxsw_core),
		},
		.mlxsw_core = mlxsw_core,
		.device = device,
	};
	struct mlxsw_linecard *linecard = device->linecard;
	int err;

	mutex_lock(&linecard->lock);
	if (!linecard->ready) {
		NL_SET_ERR_MSG_MOD(extack, "Linecard is not ready to flash firmware");
		err = -EINVAL;
		goto unlock;
	}
