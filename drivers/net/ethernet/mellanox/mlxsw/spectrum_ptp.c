				    ...
				    struct sk_buff *skb, u64 timestamp)
{
	...
	if (skb && unmatched && unmatched->timestamp) {
		...
		/* Either there is no entry to match, or the existing unmatched
		 * entry holds an older entry of the same type: either an skb if
		 * we are handling skb, or a timestamp if we are handling
	...
	if (skb)
		mlxsw_sp1_ptp_packet_finish(mlxsw_sp, skb, key.local_port,
					    key.ingress, NULL);
	if (unmatched)
