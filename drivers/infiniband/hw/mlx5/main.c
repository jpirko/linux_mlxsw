...
static void mlx5_netdev_notifier_add(struct mlx5_roce *roce,
				     struct net_device *netdev)
{
	...
	if (WARN_ON(err))
		roce->nb.notifier_call = NULL;
}

static void mlx5_netdev_notifier_remove(struct mlx5_roce *roce,
					struct net_device *netdev)
{
	if (!roce->nb.notifier_call)
		return;
	unregister_netdevice_notifier_dev_net(netdev, &roce->nb, &roce->nn);
	roce->nb.notifier_call = NULL;
}
...
{
	struct mlx5_roce *roce = container_of(nb, struct mlx5_roce, adev_nb);
	struct net_device *netdev = data;

	switch (event) {
	case MLX5_ADEV_EVENT_NETDEV_ADDED:
		mlx5_netdev_notifier_add(roce, netdev);
		break;
	case MLX5_ADEV_EVENT_NETDEV_REMOVED:
		mlx5_netdev_notifier_remove(roce, netdev);
		break;
	default:
		return NOTIFY_DONE;
	}
...
static void mlx5_adev_netdev_track(struct mlx5_ib_dev *dev, u32 port_num)
{
	struct mlx5_roce *roce = &dev->port[port_num].roce;

	if (dev->is_rep)
		return;
	roce->adev_nb.notifier_call = mlx5e_adev_notifier_event;
	mlx5_adev_notifier_register(dev->mdev, &roce->adev_nb);
}

static void mlx5_adev_netdev_untrack(struct mlx5_ib_dev *dev, u32 port_num)
{
	struct mlx5_roce *roce = &dev->port[port_num].roce;

	if (dev->is_rep)
		return;
	mlx5_adev_notifier_unregister(dev->mdev, &roce->adev_nb);
}
				      ...
				      struct mlx5_ib_multiport_info *mpi)
{
	...
	mlx5_adev_netdev_untrack(ibdev, port_num);
	spin_lock(&port->mp.mpi_lock);
				    ...
				    struct mlx5_ib_multiport_info *mpi)
{
	...
	mlx5_adev_netdev_track(ibdev, port_num);
...
static int mlx5_ib_roce_init(struct mlx5_ib_dev *dev)
{
	...
	if (ll == IB_LINK_LAYER_ETHERNET) {
		...
		mlx5_adev_netdev_track(dev, port_num);
	...
	return 0;
cleanup:
	mlx5_adev_netdev_untrack(dev, port_num);
	return err;
}
