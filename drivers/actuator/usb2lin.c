...
#define U2L_USBMSG_REQUEST_TYPE (USB_RECIP_INTERFACE | USB_TYPE_CLASS)
#define U2L_USBMSG_REQUEST_TYPE_IN (U2L_USBMSG_REQUEST_TYPE | USB_DIR_IN)
#define U2L_USBMSG_REQUEST_TYPE_OUT (U2L_USBMSG_REQUEST_TYPE | USB_DIR_OUT)
...
static void u2l_actuator_vector_fill(struct u2l *u2l,
				     ...
				     struct u2l_status *status)
{
	...
	if (status->move_magic) {
		...
		dir = status->move_flags & U2L_STATUS_MOVE_FLAGS_DIR ?
		      ACTUATOR_DIR_NEGATIVE :
		      ACTUATOR_DIR_POSITIVE;
		actuator_vector_dir_set(vector, dir);
	} else {
		actuator_vector_stop_set(vector);
	}
	value = le16_to_cpu(status->height) * U2L_HEIGHT_MULTIPLIER;
	actuator_vector_inc_value_set(vector, value);
}

static int u2l_actuator_vector_get(struct actuator *act,
				   struct actuator_vector *vector)
{
	...
	if (err)
		return err;
	u2l_actuator_vector_fill(u2l, vector, &status);
	return 0;
}
...
{
	struct actuator_vector *vector = actuator_clean_tmp_vector(u2l->act);

	if (!!status->move_magic != !!u2l->last_status.move_magic) {
		u2l_actuator_vector_fill(u2l, vector, status);
		actuator_notify_vector(u2l->act, vector);
	}
}
...
static int u2l_actuator_move(struct actuator *act,
			     struct actuator_vector *vector)
{
	struct u2l *u2l = actuator_priv(act);
	u32 value;
	...
	value = actuator_vector_value_get(vector);
...
static const struct actuator_vector_set_op u2l_actuator_vector_set_ops[] = {
	ACTUATOR_VECTOR_SET_OP(ACTUATOR_VECTOR_MOVE | ACTUATOR_VECTOR_VALUE,
			       u2l_actuator_move),
	ACTUATOR_VECTOR_SET_OP(ACTUATOR_VECTOR_STOP, u2l_actuator_stop),
};
...
static const struct actuator_ops u2l_actuator_ops = {
	...
	.vector_get = u2l_actuator_vector_get,
	.vector_set_ops = u2l_actuator_vector_set_ops,
	.vector_set_ops_count = ARRAY_SIZE(u2l_actuator_vector_set_ops),
};
