...
static int check_stats_nodelta(struct objagg *objagg)
{
	...
	if (stats->obj_stats_count != NUM_KEYS) {
		pr_err("Stats: Unexpected object count (%u expected, %u returned)\n",
		       NUM_KEYS, stats->obj_stats_count);
		err = -EINVAL;
		goto stats_put;
	}

	for (i = 0; i < stats->obj_stats_count; i++) {
		if (stats->obj_stats[i].user_count != 2) {
			pr_err("Stats: incorrect user count\n");
			err = -EINVAL;
			goto stats_put;
		}
		if (stats->obj_stats[i].delta_user_count != 2) {
...
static const struct action_item action_items[] = {
	...
	},	/* r: 5, 7, 30		d: 8^7, 8^7 */
	{
		...
		EXPECT_STATS(5, ROOT(1, 3), ROOT(1, 2), ROOT(1, 1),
				DELTA(2), DELTA(1)),
	},	/* r: 5, 7, 30		d: 6^5, 8^7, 8^7 */
	{
		8, ACTION_GET, EXPECT_DELTA_SAME, EXPECT_ROOT_SAME,
		EXPECT_STATS(5, ROOT(1, 4), ROOT(1, 2), ROOT(1, 1),
				DELTA(3), DELTA(1)),
	},	/* r: 5, 7, 30		d: 6^5, 8^7, 8^7, 8^7 */
	{
	...
	{
		8, ACTION_PUT, EXPECT_DELTA_SAME, EXPECT_ROOT_SAME,
		EXPECT_STATS(5, ROOT(1, 2), ROOT(1, 2), ROOT(1, 1),
				DELTA(1), DELTA(1)),
	},	/* r: 5, 7, 30		d: 6^5, 8^7 */
	{
		8, ACTION_PUT, EXPECT_DELTA_DEC, EXPECT_ROOT_SAME,
		EXPECT_STATS(4, ROOT(1, 2), ROOT(1, 1), ROOT(1, 1),
				DELTA(1)),
	},	/* r: 5, 7, 30		d: 6^5 */
	{
		8, ACTION_GET, EXPECT_DELTA_INC, EXPECT_ROOT_SAME,
		EXPECT_STATS(5, ROOT(1, 3), ROOT(1, 1), ROOT(1, 1),
				DELTA(1), DELTA(1)),
	},	/* r: 5, 7, 30		d: 6^5, 8^5 */
	{
			      ...
			      const struct action_item *action_item)
{
	...
	if (stats->obj_stats_count != action_item->expect_obj_stats_count) {
		pr_err("Key %u: Stats: Unexpected object count (%u expected, %u returned)\n",
		       key_id, action_item->expect_obj_stats_count,
		       stats->obj_stats_count);
		err = -EINVAL;
		goto stats_put;
	}

	for (i = 0; i < stats->obj_stats_count; i++) {
		if (stats->obj_stats[i].is_root !=
		    action_item->expect_obj_stats[i].is_root) {
			pr_err("Key %u: Stats: incorrect root/delta indication\n",
			       key_id);
			err = -EINVAL;
			goto stats_put;
		}
		if (stats->obj_stats[i].user_count !=
		    action_item->expect_obj_stats[i].user_count) {
			pr_err("Key %u: Stats: incorrect user count\n",
			       key_id);
			err = -EINVAL;
			goto stats_put;
		}
		if (stats->obj_stats[i].delta_user_count !=
		    action_item->expect_obj_stats[i].delta_user_count) {
			pr_err("Key %u: Stats: incorrect delta user count\n",
			       key_id);
			err = -EINVAL;
			goto stats_put;
		}
				  ...
				  bool inverse)
{
	...
	err = check_expect_stats(objagg, action_item);
	if (err)
		goto errout;
