...
static int objagg_stats_sort_cmp_func(const void *a, const void *b)
{
	const struct objagg_obj_stats *stats1 = a;
	const struct objagg_obj_stats *stats2 = b;

	if (stats1->is_root != stats2->is_root)
		return stats2->is_root - stats1->is_root;
	if (stats1->delta_user_count != stats2->delta_user_count)
		return stats2->delta_user_count - stats1->delta_user_count;
	return stats2->user_count - stats1->user_count;
}
 ...
 * otherwise it returns pointer error using ERR_PTR macro.
 */
struct objagg_stats *objagg_stats_get(struct objagg *objagg)
{
	...
	alloc_size = sizeof(*objagg_stats) +
		     sizeof(objagg_stats->obj_stats[0]) * objagg->obj_count;
	...
	list_for_each_entry(objagg_obj, &objagg->obj_list, list) {
		memcpy(&objagg_stats->obj_stats[i], &objagg_obj->stats,
		       sizeof(objagg_stats->obj_stats[0]));
		objagg_stats->obj_stats[i].is_root = !objagg_obj->parent;
		i++;
	}
	objagg_stats->obj_stats_count = i;

	sort(objagg_stats->obj_stats, objagg_stats->obj_stats_count,
	     sizeof(struct objagg_obj_stats), objagg_stats_sort_cmp_func, NULL);
 ...
 * Note: all locking must be provided by the caller.
 */
void objagg_stats_put(struct objagg_stats *objagg_stats)
{
	kfree(objagg_stats);
}
